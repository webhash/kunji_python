...
print(note:='but once a parameter is assigned a default value, all parameters after it  must be assigned a default values too')
...
print(note:='although iteratable unpacking returns a list, in context of function it returns tuple, which makes sense as python won`t want you to change the arguments passed by user') 
...
sums_1(1,2,3,4,5)
print(note:='one can use any valid param name instead of args, but sticking to standard nomenclature makes your code more readable')
...
print()
print(note:='if nothing is passed we use the default flag value, if we had not assigned the False as default value we will get TypeError')
sum_3()
...
print(note:='remeber the double astricks ? yes **')
