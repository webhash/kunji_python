...
print(hex(id(some_list)))
print(
    'As you can see above that mod_function had the reference to the some_list, it modified it, and we see the change outside the function because list is of mutable type')
...
print(
    note := 'python has a help function to shows us details and providea glimpse into the functionality of thing under consideration')
...
print()
print(
    note := 'we can add docstring to our own functions so that when help is called on it, we print the requested information')
...
print()
print(
    note := 'did you notice the __doc__ attribute in above result?, if we print it we shall see the same content that help throws')
...
def to_upper_with_annotation(
        input: 'annotation for input, we expect a string as input') -> 'annotation for return value, we dont return anything from this function':
    # this is a docstring that shall be consumed by help function
    '''
	prints the upper case of the string passed
	if no string passed we shall update the user that we expect a string input'
