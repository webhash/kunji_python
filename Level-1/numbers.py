...
print(
    note := 'int type need not be a number of base 10, python has constructor to initiate the int with different type of base')
...
print(
    note := 'True and false will point to a fixed memory address, when we do comparision the result would be True/false pointing to same memory address')
print(hex(id(10 > 1)))
print(hex(id(10 < 1)))
...
print(
    note := 'unlike int float has a single constructor that takes a number or string representation of number and returns a float value')
...
print(
    note := 'float doesnt necessarily have the exact representation for all numbers, the magic of ration of 2s come into picture')
print(note := 'Floating-point numbers are represented in computer hardware as base 2 (binary) fractions.')

fa = 0.1 + 0.1 + 0.1
fb = 0.3
print(
    note := 'though two numbers should have same value but if we check out enough precision point, we will see the difference')
print(fa)
print(fb)
assert fa != fb
